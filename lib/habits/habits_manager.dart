...
import 'package:timecraft/model/backup.dart';
import 'package:timecraft/model/habit_data.dart';
import 'package:timecraft/model/habo_model.dart';
import 'package:timecraft/notifications.dart';
  ...
  resetNotifications(List<Habit> habits) {
    for (var element in habits) {
      if (element.habitData.notification) {
        ...
        setHabitNotification(data.id!, data.notTime, 'TimeCraft', data.title);
      }
    }
  }
  ...
  addHabit(
    ...
    _haboModel.insertHabit(newHabit).then(
      (id) {
        ...
        if (notification) {
          setHabitNotification(id, notTime, 'TimeCraft', title);
        } else {
  ...
  editHabit(HabitData habitData) {
    ...
    if (habitData.notification) {
      setHabitNotification(
          habitData.id!, habitData.notTime, 'TimeCraft', habitData.title);
    } else {
